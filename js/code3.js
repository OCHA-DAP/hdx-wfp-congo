...
function instanciateData(data) {
    ...
    data.forEach(function (d) {
        d['#indicator+secondary+school+m'] = checkIntData(d['#indicator+secondary+school+m']);
    });
    cf = crossfilter(data);


    departementDim = cf.dimension(function (d) {
        return d['#adm1+name+origine'];
    });
    ...
    stats = cf.groupAll().reduce(
        function (p, v) {
            p.totIndividus += +parseInt(v["#affected+num"]);
            p.totMenages += +parseInt(v["#affected+households"]);
            p.totHommes += +parseInt(v["#affected+idp+m"]);
            p.totFemmes += +parseInt(v["#affected+idp+f"]);

            p.totVieux += +parseInt(v["#affeced+elderly"]);
            p.totVieuxH += +parseInt(v["#affeced+elderly+m"]);
            p.totVieuxF += +parseInt(v["#affeced+elderly+f"]);

            p.totPrimaire += +parseInt(v["#indicator+primary+school"]);
            p.totPrimaireH += +parseInt(v["#indicator+primary+school+m"]);
            p.totPrimaireF += +parseInt(v["#indicator+primary+school+f"]);
            console.log(parseInt(v["#indicator+primary+school+m"]));

            p.totCollege += +parseInt(v["#indicator+secondary+school"]);
            p.totCollegeH += +parseInt(v["#indicator+secondary+school+m"]);
            p.totCollegeF += +parseInt(v['#indicator+secondary+school+f']);

            p.totLycee += +parseInt(v["#indicator+college+school"]);
            p.totLyceeH += +parseInt(v["#indicator+college+school+m"]);
            p.totLyceeF += +parseInt(v["#indicator+college+school+f"]);

            p.handicap += +parseInt(v["#indicator+handicap"]);
            p.enceintes += +parseInt(v["#indicator+pregnancy"]);
            p.veuves += +parseInt(v["#indicator+widow"]);
            p.orphelins += +parseInt(v["#indicator+orphan"]);
            p.malades += +parseInt(v["#indicator+sickness"]);
            p.seuls += +parseInt(v["#indicator+infants"]);
...
var idpCall = $.ajax({
    type: 'GET',
    url: 'https://proxy.hxlstandard.org/data.json?strip-headers=on&url=https://docs.google.com/spreadsheets/d/1TJtXfd5E77QAp96GmAhczQ5YX7t-ku4DWhzHOlVS0QQ/edit&force=on',
    dataType: 'json',
});


$.when(idpCall).then(function (idpArgs) {
    donnees = hxlProxyToJSON(idpArgs);
    generateCharts('');
})
